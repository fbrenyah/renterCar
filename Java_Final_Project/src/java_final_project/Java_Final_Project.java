...
public class Java_Final_Project extends Applet implements ActionListener
{
    ...
    public void init() {
        ...
        for(int temp1=0; temp1<6; temp1++)
            compImg[temp1] = this.getImage(this.getCodeBase(),compactPic[temp1]);
        for(int temp2=0; temp2<5; temp2++)
            interImg[temp2] = this.getImage(this.getCodeBase(),interPic[temp2]);
    ...
    public void paint(Graphics g) {
     ...
     g.setColor(Color.WHITE);
        ...
        //draw default images
        g.drawImage(compImg[temp1],40,110,210,120,this);
        g.drawImage(interImg[temp2],40,240,210,120,this);
        g.drawImage(fullImg[temp3],40,370,210,120,this);
        
        //"rent" function
        if(reserveONE==true)
            g.drawImage(compImg[temp1],430,110,210,110,this);
        if(reserveTWO==true)
            g.drawImage(interImg[temp2],430,240,210,110,this);
        if(reserveTHREE==true)
            g.drawImage(fullImg[temp3],430,370,210,110,this);
    }
    public void actionPerformed(ActionEvent ae)
    {
        ...
        {
            n = daysDropBox.getSelectedIndex() + 1;
            if(reserveONE==true)
            {
                totalBill=cost1; totalBill*=n;
                CAR = "SubCompact";  
                carMOD = compactPic[temp1];//get filename of current pic
